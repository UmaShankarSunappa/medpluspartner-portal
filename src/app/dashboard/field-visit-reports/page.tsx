...
export default function FieldVisitReportsPage() {
  ...
  const handleDownload = (report: FieldVisitReport) => {
    ...
    const rows = [
        ...
        ["Store Environment", report.ratings.storeEnvironment.toString(), report.remarks.storeEnvironment || ""],
        ["Staff Grooming", report.ratings.staffGrooming.toString(), report.remarks.staffGrooming || ""],
        ["Staff Quality", report.ratings.staffQuality.toString(), report.remarks.staffQuality || ""],
        ["Staff Present", report.staffPresent.toString(), ""],
        ["PVT Label Promotions – Pharma", report.ratings.pvtLabelPharma.toString(), report.remarks.pvtLabelPharma || ""],
        ["PVT Label Promotions – Non-Pharma", report.ratings.pvtLabelNonPharma.toString(), report.remarks.pvtLabelNonPharma || ""],
        ["TO’s Replenishment Status", report.replenishmentStatus, ""],
        ["Outstanding Payments", report.outstandingPayments, report.outstandingPaymentsRemarks || ""],
                    ...
                    {fieldVisitReportsData.map((report) => {
                        ...
                        return (
                            <TableRow key={report.id}>
                                ...
                                <TableCell>
                                    <p className="w-48 truncate text-sm text-muted-foreground">{getTruncatedRemarks(report)}</p>
                                </TableCell>
